...
import com.braunster.androidchatsdk.firebaseplugin.firebase.fcm.FcmNotificationBuilder;
...
public abstract class BFirebaseNetworkAdapter extends AbstractNetworkAdapter {
    ...
    protected void pushToUsers(BMessage message, List<BUser> users){
        ...
        String receiverFirebaseToken = users.get(0).getMetaToken();     //Added by zhang
...
//        pushHandler.pushToChannels(channels, data);       //removed by zhang

//      Added by zhang
        String senderUid = message.getBUserSender().getEntityID();
        String firebaseToken = message.getThread().getUsers().get(0).getMetaToken();
        // send push notification to the receiver
        sendPushNotificationToReceiver( sender,
                messageText,
                senderUid,
                firebaseToken,
